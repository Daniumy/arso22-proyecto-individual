			...
			try {
				...
				if (!roles.contains(AvailableRoles.valueOf(userRole))) {
					throw new WebApplicationException(
							Utils.buildForbiddenResponse(userRole, method.getResource().getPath().getValue()));
				}

				// Establece un nuevo contexto de seguridad que será inyectado en el servicio
				// REST
				...
				throw new WebApplicationException(Utils.buildUnauthorizedResponse("El token de acceso no es valido",
