...
public class Utils {
	public static void writeResponseJSON(HttpServletResponse response, int codigoHttp, String mensaje)
			throws IOException {
	...
	public static String jwtResponse(String jwt) {
		...
		return Json.createObjectBuilder().add("status", "Authorized").add("token_type", "Bearer").add("token", jwt)
				.add("expires_in", "3600").build().toString();
	}
}
