...
public class JwtUtils {
	...
	public static String createJWT(String userId, Map<String, String> userInfo) {
		...
		return Jwts.builder().setId(UUID.randomUUID().toString()).setIssuer(ISSUER)
				.setExpiration(new Date(System.currentTimeMillis() + EXPIRATION_TIME)).setSubject(userId)
				.claim("usuario", userInfo.get("email")).claim("rol", userInfo.get("rol"))
